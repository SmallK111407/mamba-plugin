...
class Setting {
  constructor () {
    /** 默认设置 */
    this.defPath = `${_path}/plugins/mamba-plugin/def/`
    this.def = {}

    /** 用户设置 */
    this.configPath = `${_path}/plugins/mamba-plugin/config/`
    this.config = {}

    this.dataPath = `${_path}/plugins/mamba-plugin/data/`
    this.data = {}
  ...
  getFilePath (app, type) {
    if (type === 'def') return `${this.defPath}${app}.yaml`
      ...
      try {
        ...
        logger.error(`曼巴插件缺失默认文件[${app}]${error}`)
      }
  ...
  watch (file, app, type = 'def') {
    ...
    watcher.on('change', path => {
      ...
      logger.mark(`[曼巴插件][修改配置文件][${type}][${app}]`)
      if (this[`change_${app}`]) {
        this[`change_${app}`]()
      }
    })
